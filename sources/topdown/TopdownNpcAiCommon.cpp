        ...
        TopdownNpcMoveOwner owner)
{
    ...
    auto computePathLengthFrom = [](Vector2 start, const std::vector<Vector2>& points) -> float {
        if (points.empty()) {
            return 0.0f;
        }

        float total = TopdownLength(TopdownSub(points[0], start));
        for (int i = 0; i + 1 < static_cast<int>(points.size()); ++i) {
            total += TopdownLength(TopdownSub(points[i + 1], points[i]));
        }
        return total;
    };

    auto computeRemainingCurrentPathLength = [](const TopdownNpcRuntime& npcRuntime) -> float {
        const TopdownNpcMoveState& move = npcRuntime.move;
        ...
        float total = TopdownLength(
                TopdownSub(move.pathPoints[move.currentPoint], npcRuntime.position));

        for (int i = move.currentPoint; i + 1 < static_cast<int>(move.pathPoints.size()); ++i) {
            total += TopdownLength(
                    TopdownSub(move.pathPoints[i + 1], move.pathPoints[i]));
        }
    ...
    // ---------------------------------------------------------------------
    // AI repath hysteresis:
    // Do not replace an existing chase path unless the new one is clearly
    // better, the target moved a lot, or the old path is basically done.
    // This prevents "flip to the other side of the lake" oscillation.
    // ---------------------------------------------------------------------
    if (owner == TopdownNpcMoveOwner::Ai &&
        npc.move.active &&
        npc.move.owner == TopdownNpcMoveOwner::Ai) {

        const float currentRemaining = computeRemainingCurrentPathLength(npc);
        const float newLength = computePathLengthFrom(npc.position, pathPoints);

        const float targetShift =
                TopdownLength(TopdownSub(resolvedEndPos, npc.move.finalTarget));

        const bool oldPathAlmostDone =
                currentRemaining <= 80.0f ||
                npc.move.currentPoint >= static_cast<int>(npc.move.pathPoints.size()) - 1;

        const bool targetMovedALot = targetShift >= 140.0f;

        // Require the new path to be meaningfully better before switching.
        // This avoids route flapping around large obstacles.
        const float improvementRequired = 120.0f;
        const bool newPathClearlyBetter =
                newLength + improvementRequired < currentRemaining;

        const bool currentPathInvalid =
